        ...
        <>
            <motion.nav
                ...
                className={`fixed top-0 left-0 w-full z-[60] transition-all duration-500 ${isScrolled
                    ? "bg-slate-950/80 backdrop-blur-xl py-3 border-b border-slate-800/50 shadow-2xl shadow-cyan-500/10"
                    : "bg-transparent py-6"
                    }`}
            >
                <div className="container mx-auto px-6 md:px-12 lg:px-24 flex justify-between items-center">
                    ...
                    <Link
                        ...
                        className="group flex items-center gap-2 text-2xl font-black text-white tracking-tighter z-10"
                    >
                        <span className="bg-cyan-500 text-slate-950 px-2 py-0.5 rounded-lg transform -rotate-3 group-hover:rotate-0 transition-transform duration-300 font-sans">
                            W
                        </span>
                        <span className="hover:text-cyan-400 transition-colors duration-300">
                            AHAB<span className="text-cyan-500">ALI</span>
                        </span>
                    </Link>
                    ...
                    <div className="hidden lg:flex items-center space-x-1">
                        {navLinks.map((link) => (
                            ...
                            >
                                {link.name}
                                <span
                                    className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-cyan-500 transition-all duration-300 ${activeLink === link.path ? "w-4" : "w-0 group-hover:w-2"
                                        }`}
                                ></span>
                            </a>
                        ))}
                    ...
                    {/* Mobile Menu Toggle - HIGHEST Z-INDEX & POINTER EVENTS FIX */}
                    <button
                        onClick={() => setIsMenuOpen(!isMenuOpen)}
                        className="lg:hidden relative z-[70] p-3 text-white bg-slate-900/80 backdrop-blur-md rounded-xl border border-slate-700 hover:border-cyan-500 transition-all duration-300 focus:outline-none shadow-lg"
                        ...
                        >
                            ...
                            <div className="p-6 flex justify-between items-center border-b border-slate-900">
                                ...
                                >
                                    ...
                                    <span>AHAB<span className="text-cyan-500">ALI</span></span>
                                </Link>
                                <button
                                    onClick={() => setIsMenuOpen(false)}
                            ...
                            <div className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
                                {navLinks.map((link, idx) => (
                                    ...
                                    >
                                        <div
                                            className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${activeLink === link.path
                                                ? "bg-cyan-400 scale-100"
                                                : "bg-slate-700 scale-0 group-hover:scale-100 group-hover:bg-slate-500"
                                                }`}
                                        />
